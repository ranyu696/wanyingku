...
// 播放页（纯客户端）：详情页点「播放/某一集」后进来，这里才有播放器。
// 短剧（kind=6）走 9:16 竖屏沉浸式信息流（ShortDramaFeed），其余横屏：播放器铺满 + 线路/选集。
export default function WatchPage() {
  const params = useParams<{ id: string }>();
  const id = params.id;
  const sp = useSearchParams();
  const searchLine = sp.get("line") != null ? Number(sp.get("line")) : undefined;
  const searchEp = sp.get("ep") != null ? Number(sp.get("ep")) : undefined;
  ...
  const [mounted, setMounted] = useState(false); // 纯客户端：SSR 与首屏统一渲染 Loading，避开播放器水合不一致
  ...
  useEffect(() => {
    ...
    if (searchLine != null && ls[searchLine]) {
      setLineIdx(searchLine);
    }
    if (searchEp != null) {
      setEpIdx(searchEp);
      setResumePos(0);
    ...
    <Box sx={{ pb: 3 }}>
      ...
      <Box sx={{ px: { xs: 1.5, md: 2 }, mt: 2 }}>
        ...
        {ep ? (
          <Box sx={{ mt: 2 }}>
            {!marking ? (
              <Button size="small" variant="text" onClick={openMarking} sx={{ color: "text.secondary" }}>
                ⏱ 标记片头片尾
              ...
              >
                ...
                <Stack direction="row" spacing={1} sx={{ flexWrap: "wrap", gap: 1, mb: 1 }}>
                  <Button size="small" variant="outlined" onClick={() => setIntroMark(Math.floor(curRef.current))}>
                    片头结束 = 此刻{introMark > 0 ? ` (${fmt(introMark)})` : ""}
                  </Button>
                  <Button size="small" variant="outlined" onClick={() => setOutroMark(Math.floor(curRef.current))}>
