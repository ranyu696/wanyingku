...
import { useCreateRequest, useRequestsList, useRequestVote } from "@/lib/hooks";
import { KIND_LABELS, REQ_STATUS } from "@/lib/types";
  ...
  return (
    <Box sx={{ px: { xs: 1.5, md: 2 }, pt: 1.5, pb: 2 }}>
      ...
      <Stack direction="row" spacing={1} sx={{ mb: 2, overflowX: "auto", "&::-webkit-scrollbar": { display: "none" } }}>
        {tabs.map(([v, l]) => (
          <Chip key={v} label={l} color={status === v ? "primary" : "default"} onClick={() => setStatus(v)} />
        ))}
      </Stack>
